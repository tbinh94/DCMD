...
def normalize_pose(pose_data, **kwargs):
    ...
    symm_range = kwargs.get('symm_range', True) # Is there a symmetrical range
    sub_mean = kwargs.get('sub_mean', True) # Is the mean subtracted
    return_mean = kwargs.get('return_mean', True) # Whether to return the mean value
    vid_res_wconf = vid_res + [1]
    norm_factor = np.array(vid_res_wconf)
    pose_data_normalized = pose_data / norm_factor # Normalize the keypoint values to the range of [0,1]
...
def apply_pose_transform(pose, trans_mat):
    ...
    pose_transformed_wo_conf = np.einsum(einsum_str, pose_w_ones, trans_mat) # 乘积求和
...
class StdScaler:
    def __init__(self, stds=3):
        self.stds = stds
        self.mu = None # Used to save the mean value
        self.sigma = None # Used to save standard deviation
