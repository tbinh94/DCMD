...
def compute_bounding_box(keypoints, video_resolution, return_discrete_values=True):
    ...
    x, y = x[x != 0.0], y[y != 0.0] # Remove points with a value of 0
    try:
...
class Trajectory:
    ...
    def _from_image_to_centre_bounding_box(coordinates, video_resolution):
        ...
        for idx, kps in enumerate(coordinates):
            ...
            coordinates[idx] = kps
        return coordinates
    # Determine if the length is less than the minimum trajectory length
    def is_short(self, input_length, input_gap, pred_length=0):
        min_trajectory_length = input_length + input_gap * (input_length - 1) + pred_length # 31

        return len(self) < min_trajectory_length

    # Fill in missing steps with the weighted average of the nearest non missing steps
...
def load_trajectories(trajectories_path, debug, split='train'):
    ...
    for folder_name in folder_names:
        ...
        for csv_file_name in csv_file_names:
            ...
            trajectory_frames, trajectory_coordinates = trajectory[:, 0].astype(np.int32), trajectory[:, 1:] # 第一列为帧、其余列为坐标
...
# Use for scaling input trajectory data
