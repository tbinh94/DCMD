...
def data_of_combined_model(**args):
    ...
    if include_global:
        ...
        if normalize_pose == True:
            # --- START: Logic được viết lại cho global scaler_path ---
            if split == 'train':
                ...
                # Sử dụng một file scaler riêng cho global khi test
                scaler_path = '/kaggle/input/checkpoint/kaggle/working/checkpoints/HR-Avenue/train_experiment/global_robust.pickle'
                global_scaler = load_scaler(scaler_path)
            # --- END: Logic được viết lại cho global scaler_path ---
    ...
    if normalize_pose == True:
        # --- START: Logic được viết lại cho local scaler_path ---
        if split == 'train':
            ...
            scaler_path = '/kaggle/input/checkpoint/kaggle/working/checkpoints/HR-Avenue/train_experiment/local_robust.pickle'
            local_scaler = load_scaler(scaler_path)
        # --- END: Logic được viết lại cho local scaler_path ---
